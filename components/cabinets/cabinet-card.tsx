...
export function CabinetCard({ cabinet, onClick }: CabinetCardProps) {
  ...
  return (
    <button
      ...
      className={cn(
        "group relative w-full rounded-xl border border-gray-200 bg-white text-left",
        "cursor-pointer p-5 shadow-sm transition-all duration-200",
        config.cardGlow,
        config.borderClass,
    ...
    >
      ...
      <div className="mb-4 flex items-start justify-between">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <Badge
            variant="outline"
            className={cn(
              "h-5 border px-2 py-0 text-[11px] font-medium",
              config.badgeClass,
            )}
          >
            {config.label}
      ...
      {/* Cabinet name */}
      <p className="mb-1 text-sm leading-tight font-semibold text-gray-900">
        {cabinet.name}
      </p>
      {cabinet.location && (
        <p className="mb-4 truncate text-[12px] text-gray-400">
          {cabinet.location}
        </p>
      )}
      ...
      <div className="flex items-center gap-4 border-t border-gray-100 pt-3">
        ...
        {cabinet._count?.active_sessions != null &&
          cabinet._count.active_sessions > 0 && (
            <div className="flex items-center gap-1.5 text-amber-600">
              <Users className="h-3.5 w-3.5" />
              <span className="text-[12px]">
                {cabinet._count.active_sessions} activo
                {cabinet._count.active_sessions !== 1 ? "s" : ""}
              </span>
            </div>
          )}
      </div>
